      ...
      <form onSubmit={handlePostCreate} className="flex flex-col gap-2 border rounded-md p-5">
        ...
        <div className="p-5">
          {posts?.map((post,indx) =>{
            ...
            <div aria-disabled={post.likes.includes(username)} onClick={() => !post.likes.includes(username) && handlePostLike(post.id)} className={`flex items-center gap-2 ${ post.likes.includes(username) ? "opacity-45 cursor-not-allowed" : "cursor-pointer"
  }`}
>
